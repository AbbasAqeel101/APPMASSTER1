...
class AppDelegate: NSObject, UIApplicationDelegate {
	...
	/// app/section you add or edit on the dashboard shows up here automatically,
	/// without needing another app build.
	private func _addCompanySource() {
		guard
			UserDefaults.standard.bool(forKey: "feather.didAddCompanySource") == false,
			let companySourceURL = URL(string: "https://seemtaruyoixqdfdqwfv.supabase.co/functions/v1/app-catalog-feed")
		else {
			return
		}
		...
		Storage.shared.addSource(
			companySourceURL,
			name: "AppMaster",
			identifier: "com.appmaster.companystore"
		) { error in
			if let error {
				Logger.misc.error("Failed to add company source: \(error)")
				return
			}
			UserDefaults.standard.set(true, forKey: "feather.didAddCompanySource")
		}
	}
